...
namespace bftEngine {
namespace impl {
class IInternalBFTCustomer {
 public:
  virtual ~IInternalBFTCustomer() {}
  virtual NodeIdType getClientId() const = 0;
  virtual void sendRequest(uint8_t flags, uint32_t requestLength, const char* request, const std::string& cid) = 0;
  virtual uint32_t numOfConnectedReplicas(uint32_t clusterSize) = 0;
  virtual bool isUdp() = 0;
};

class InternalBFTCustomer : public IInternalBFTCustomer {
 public:
  InternalBFTCustomer(const int& id, const NodeIdType& nonInternalNum, std::shared_ptr<MsgsCommunicator>& msgComm);
